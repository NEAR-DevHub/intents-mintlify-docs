...
Before you can swap or transfer tokens on the Verifier contract, you need to deposit them. Depositing moves tokens from your NEAR account into the contract's internal ledger, where they can be used for [intents](/integration/verifier-contract/intent-types-and-execution). Only [NEP-141](https://nomicon.io/Standards/Tokens/FungibleToken/Core) fungible tokens and [NEP-171](https://nomicon.io/Standards/Tokens/NonFungibleToken/Core) non-fungible tokens are accepted.
...
For a list of NEP-141 token contract addresses deposited into the Verifier contract, run:
...
|-----------|-------------|
| `<token-contract>` | The token contract you're depositing from (e.g., `wrap.near` for wNEAR, `eth.bridge.near` for ETH). Use the [command above](#depositing-fungible-tokens-nep-141) to list available contract addresses. |
